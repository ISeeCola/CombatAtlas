...
for (const row of accepted) {
  ...
  const html = `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>${esc(title)}</title><style>body{max-width:900px;margin:56px auto;padding:0 24px;background:#f3eee6;color:#181715;font:17px/1.75 system-ui}h1{font-size:42px;line-height:1.15}small{color:#835344}section{border-top:1px solid #bbb;padding-top:18px;margin-top:28px}</style><h1>${esc(title)}</h1><p><small>${esc(original)} · ${esc(row['作者'])} · ${esc(row['来源'])} · ${esc(row['正式总分'])}/100</small></p><p><a href="${esc(row['规范 URL'])}">访问原文</a></p><section><h2>摘要</h2><p>${esc(row['摘要'])}</p></section><section><h2>核心方法</h2><p>${esc(row['核心方法']).replaceAll('\n','<br>')}</p></section><section><h2>适用边界</h2><p>${esc(row['适用边界'])}</p></section><section><h2>验证实验</h2><p>${esc(row['可迁移假设'])}</p><p>${esc(row['最小验证动作'])}</p></section></html>`;
